...
function patch(data: object, root: string = "", rootValue: string = "", types: FieldTypeMap) {
    ...
    let addResult = false;
    if (root !== "" && rootValue !== "") addResult = addTypename(data, root, rootValue);
...
function artificiallyAddNestedFields(typeDefinitions: TypeDefinitions) {
    ...
    rootTypes.forEach(rootType => {
        const interiorTypes: Array<string> = Object.values(typeDefinitions[rootType]);
        ...
        if (nestedTypes.length > 0) {
            ...
            nestedTypes.forEach(type => {
                const typeKey =
                    Object.keys(typeDefinitions[rootType]).find(key => typeDefinitions[rootType][key] == type) || "";
...
export function typePatcherFactory(typeDefinitions: TypeDefinitions) {
    ...
    types.forEach(type => {
        const fields = Object.keys(typeDefinitions[type]).filter(field => field !== "__nested");
        const fieldTypeMap: FieldTypeMap = [];
        fields.forEach(field => fieldTypeMap.push({ field, type: typeDefinitions[type][field] }));
